    ...
    this.timerId = setInterval(() => {
      ...
      if (this.state.lines.length > 1) {
        ...
        for (let i = 1; i < lines.length; i++) {
          for (let j = 0; j < i; j++) {
            ...
            if (CollisionPoint.isCollisionPoint(result)) result.draw(ctx);
          }
        }
      }
      ...
      if (this.state.drawing) {
        const tempLine = new Line(
          new Point(this.state.temp.beginPoint.x, this.state.temp.beginPoint.y),
          new Point(this.state.temp.endPoint.x, this.state.temp.endPoint.y)
        );
